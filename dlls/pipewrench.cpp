...
void CWrench::Precache( void )
{
	...
	m_usWrench= PRECACHE_EVENT ( 1, "events/pipewrench.sc" );
	m_usWrenchLarge= PRECACHE_EVENT ( 1, "events/pipewrench2.sc" );
}
...
BOOL CWrench::Deploy( )
{
	return DefaultDeploy( "models/v_pipe_wrench.mdl", "models/p_pipe_wrench.mdl", WRENCH_DRAW, "pipewrench" );
}
...
int CWrench::Swing( int fFirst )
{
	...
	else
	{
		...
		{
			case 0:
				SendWeaponAnim( WRENCH_ATTACK1HIT );
			break;
			case 1:
				SendWeaponAnim( WRENCH_ATTACK2HIT ); 
			break;
			case 2:
				SendWeaponAnim( WRENCH_ATTACK3HIT ); 
			break;
		}
